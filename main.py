...
def offspringFromAlphaSwap(parents, alpha):
    i = 0
    j = 0
    offspring = initializePopulation(len(parents))
    while(i < len(parents)): #if iterating through parents, upper bound should be num of pairs
        mother = parents[i]
        father = parents[i+1]
        childOne = parents[i].copy() #first child is base copy of 'mother'
        childTwo = parents[i+1].copy() #second child is base copy of 'father'
                    ...
                    for num_two_to_five in range(0, 9): #0 to 8 in this category
                        for num_six_to_nine in range(0, 4): #0 to 3 in this category
                            for num_faces in range(0, 3): #0 to 2 faces
                                pivot = float((float(random.randint(0,100))/100.00))
                                if(alpha > pivot):
        ...
        # score both children and keep most fit
        childOneFitness = fitness_score_linear(childOne.strategy, 10, float(.5))
        childTwoFitness = fitness_score_linear(childTwo.strategy, 10, float(.5))

        if(childOneFitness > childTwoFitness):
            offspring[j] = childOne
        else:
            offspring[j] = childTwo

        i += 2 #iterate in multiples of 2
        j += 1 #iterate in multiples of 1
...
def selectParents(generation):
    ...
    if(len(generation.population) % 2 == 1): #if 'odd' num parents, make even num. We are extracting upper percentile of parents in regards to fitness, so we do not lose best strategy
        numParents -= 1
    for member in sortedPopulation[numParents-1:-1]: #select the most fit, upper 50% of population
